...
class Dispatcher:
    ...
    def stream_handle(self, user_name: str, message: str):
        """
        Stream responses whenever the selected agent supports it.
        Agents without streaming simply return a single response.
        """

        intent = router.detect(message)

        if intent == "chat":
            yield from chat_stream(user_name, message)
            return

        # Fallback for non-streaming agents
        yield self.handle(user_name, message)
